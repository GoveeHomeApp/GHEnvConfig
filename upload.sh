...
#修改spec文件tag
#获取版本号并显示
VersionString=`grep -E "s.version.*=" GHEnvConfig.podspec`
...
LineNumber=`grep -nE 's.version.*=' GHEnvConfig.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" GHEnvConfig.podspec
echo -e "\n${Default}================================================"
